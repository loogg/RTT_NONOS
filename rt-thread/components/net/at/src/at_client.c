...
at_response_t at_create_resp(rt_size_t buf_size, rt_size_t line_num, rt_int32_t timeout)
{
    ...
    resp->timeout_tick = timeout + rt_tick_get();
    return resp;
}
...
int at_obj_exec_cmd(at_client_t client, at_response_t resp, const char *cmd_expr, ...)
{
    ...
    if (client->status == AT_STATUS_CLI)
    {
        exec_step = 0;
        client->lock = RT_FALSE;
    }

    switch (exec_step)
    {
    case 0:
        if (client->lock == RT_TRUE)
            return -RT_EBUSY;

        client->lock = RT_TRUE;
        client->resp_status = AT_RESP_OK;
        client->resp = resp;
        client->resp_notice = RT_FALSE;
        ...
        resp->timeout_tick = rt_tick_get() + resp->timeout;
        exec_step++;
        break;
    case 1:
        if (client->resp_notice != RT_TRUE)
        {
            if ((rt_tick_get() - resp->timeout_tick) < RT_TICK_MAX / 2)
            {
...
{
    if(client->device->thread_belong != RT_DEVICE_AT_CLIENT_BELONG)
        return -RT_ERROR;
...
static int at_client_get_index(at_client_t client)
{
    int idx = 0;
    for (idx = 0; idx < AT_CLIENT_NUM_MAX; idx++)
    {
        if (client == &at_client_table[idx])
        {
            return idx;
        }
    }
...
{
    static rt_size_t read_len[AT_CLIENT_NUM_MAX] = {0};
    static char last_ch[AT_CLIENT_NUM_MAX] = {0};
    static rt_bool_t is_full[AT_CLIENT_NUM_MAX] = {RT_FALSE};
    static int read_step[AT_CLIENT_NUM_MAX] = {0};

    char ch = 0;
    int idx = at_client_get_index(client);

    RT_ASSERT(idx >= 0);

    switch (read_step[idx])
    {
    case 0:
        last_ch[idx] = 0;
        is_full[idx] = RT_FALSE;
        read_len[idx] = 0;
        ...
        break;
    case 1:
        if (at_client_getchar(client, &ch) != RT_EOK)
            return -RT_ERROR;
        if (read_len[idx] < client->recv_bufsz)
        {
            client->recv_buffer[read_len[idx]++] = ch;
            client->cur_recv_len = read_len[idx];
        }
        else
        {
            is_full[idx] = RT_TRUE;
        ...
        if ((ch == '\n' && last_ch[idx] == '\r') || (client->end_sign != 0 && ch == client->end_sign) || get_urc_obj(client))
        {
            if (is_full[idx])
            {
    ...
    case 2:
#ifdef AT_PRINT_RAW_CMD
        at_print_raw_cmd("recvline", client->recv_buffer, read_len[idx]);
#endif
        read_step[idx] = 0;
        return read_len[idx];
        break;
...
{
    static int resp_buf_len = 0;
    const struct at_urc *urc;
    static rt_size_t line_counts = 0;
...
int at_client_init(const char *dev_name,  rt_size_t recv_bufsz)
{
    ...
    if (client->device)
    {
        ...
        client->device->thread_belong = RT_DEVICE_AT_CLIENT_BELONG;
    }
    else
    {
