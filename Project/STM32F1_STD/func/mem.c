...
static void mem_param_update(void)
{
    ...
    if(stm32_flash_erase(FLASH_SAVE_ADDR, MAX_SAVE_LEN) < 0)
    {
    ...
    if(stm32_flash_write(FLASH_SAVE_ADDR, (const rt_uint8_t *)g_param.data, MAX_SAVE_LEN) < 0)
    {
...
int mem_param_init(void)
{
    ...
    stm32_flash_read(FLASH_SAVE_ADDR, (rt_uint8_t *)g_param.data, MAX_SAVE_LEN);
